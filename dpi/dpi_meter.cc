...
static dpi_meter_t *
meter_inc(uint8_t type, uint8_t *ep_mac, uint8_t *peer_ip, bool ipv4, bool *fire, bool *create) {
    dpi_meter_t key, *m;
    meter_info_t *info = &meter_info[type];
    ...
    if (likely(m != NULL)) {
        ...
        if (unlikely(fire)) {
            if (th_snap.tick - m->last_log >= info->log_timeout && m->log_count > 0) {
                dpi_ddos_log(info->log_id, m, "Packet rate %u(pps) exceeds the shreshold %u(pps)", m->last_count,
                             info->lower_limit);
