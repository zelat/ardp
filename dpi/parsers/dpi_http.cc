...
static http_method_t http_method[] = {
    {(char *)"GET",     3, HTTP_PROTO_HTTP, HTTP_METHOD_GET},
    {(char *)"PUT",     3, HTTP_PROTO_HTTP, HTTP_METHOD_PUT},
    {(char *)"POST",    4, HTTP_PROTO_HTTP, HTTP_METHOD_POST},
    {(char *)"DELETE",  6, HTTP_PROTO_HTTP, HTTP_METHOD_DELETE},
    {(char *)"HEAD",    4, HTTP_PROTO_HTTP, HTTP_METHOD_HEAD},
    {(char *)"CONNECT", 7, HTTP_PROTO_HTTP, HTTP_METHOD_NONE},
};
...
{
    http_data_t *data = (http_data_t *)parser_data;
...
{
    http_ctx_t *ctx = (http_ctx_t *)param;
    uint8_t *l = ptr, *end = ptr + len;
    int clen = 0;
...
static void buffer_body(http_ctx_t *ctx, uint8_t *ptr, int len) {
    ...
    if (unlikely(is_request(w) && w->ctype == HTTP_CTYPE_APPLICATION_XML && w->encode == HTTP_ENCODE_NONE)) {
        ...
        if (data->body_buffer == NULL) {
            data->body_buffer = (uint8_t *)malloc(2048);
        }
        if (data->body_buffer != NULL && data->body_buffer_len < 2048) {
            ...
            if (likely(th_apache_struts_re_data != NULL)) {
                int rc = pcre2_match(apache_struts_re,
                        ...
                        0, 0, (pcre2_match_data_8  *)th_apache_struts_re_data, NULL);
                if (rc >= 0) {
...
static void http_parser(dpi_packet_t *p)
{
    ...
    if (unlikely((data = (http_data_t *)dpi_get_parser_data(p)) == NULL)) {
        ...
        if ((data = (http_data_t *)calloc(1, sizeof(*data))) == NULL) {
            dpi_fire_parser(p);
            return;
        }
