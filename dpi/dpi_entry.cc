...
{
    th_packet.defrag_data = (uint8_t *)malloc(DPI_MAX_PKT_LEN);
    th_packet.asm_pkt.ptr = (uint8_t *)malloc(DPI_MAX_PKT_LEN);
    th_packet.decoded_pkt.ptr = (uint8_t *)malloc(DPI_MAX_PKT_LEN);
...
io_app_t *dpi_ep_app_map_lookup(io_ep_t *ep, uint16_t port, uint8_t ip_proto)
{
    ...
    return (io_app_t *)rcu_map_lookup(&ep->app_map, &key);
}
...
static io_app_t *ep_app_map_locate(io_ep_t *ep, uint16_t port, uint8_t ip_proto)
{
    ...
    if (unlikely(app == NULL)) {
        app = (io_app_t *)calloc(1, sizeof(*app));
...
void dpi_inject_reset_by_session(dpi_session_t *sess, bool to_server)
{
    ...
    */
    ...
    if (sess->flags & DPI_SESS_FLAG_INGRESS) {
        mac = (io_mac_t *)rcu_map_lookup(&g_ep_map, s->mac);
    } else {
        mac = (io_mac_t *)rcu_map_lookup(&g_ep_map, c->mac);
    }
    if (mac == NULL) return;
...
int dpi_recv_packet(io_ctx_t *ctx, uint8_t *ptr, int len)
{
    ...
    if (likely(th_packet.cap_len >= sizeof(struct ethhdr))) {
        ...
        } else if (cmp_mac_prefix(eth->h_source, (char *)MAC_PREFIX)) {
            mac = (io_mac_t *)rcu_map_lookup(&g_ep_map, &eth->h_source);
        } else if (cmp_mac_prefix(eth->h_dest, (char *)MAC_PREFIX)) {
            mac = (io_mac_t *)rcu_map_lookup(&g_ep_map, &eth->h_dest);
            th_packet.flags |= DPI_PKT_FLAG_INGRESS;
        } else
            // For tapped port
        ...
        if (mac_cmp(eth->h_dest, ctx->ep_mac.ether_addr_octet)) {
            ...
            mac = (io_mac_t *)rcu_map_lookup(&g_ep_map, &eth->h_source);
        }  else if (cmp_mac_prefix(ctx->ep_mac.ether_addr_octet, (char *)PROXYMESH_MAC_PREFIX)) {
            /*
