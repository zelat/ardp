...
dpi_dlp_hs_add_pattern (dpi_hyperscan_pm_t *hspm, uint8_t *pcre_pat, int patlen, uint32_t hs_flags, dpi_sig_assoc_t *sa)
{
    ...
    if (hspm->hs_patterns_num + 1 > hspm->hs_patterns_cap) {
        ...
        dpi_hyperscan_pattern_t *tmp = (dpi_hyperscan_pattern_t *)calloc(1, sizeof(dpi_hyperscan_pattern_t) * hspm->hs_patterns_cap);
        if (!tmp) {
            return;
        }
...
{
    dpi_hs_search_t *hs_search = (dpi_hs_search_t *)context;
...
static void dpi_dlp_hs_search_compile (void *context)
{
    ...
    hs_search->dlptbls = (dlptbl_t *)calloc(MAX_DP_THREADS, sizeof(dlptbl_t));
    if (hs_search->dlptbls == NULL) {
        return;
    }
    
    for (i = 0; i < MAX_DP_THREADS; i ++) {
        hs_search->dlptbls[i].tbl = (dlptbl_node_t *)calloc(hs_search->count, sizeof(dlptbl_node_t));
...
int dpi_dlp_hs_onmatch(unsigned int id, unsigned long long from, unsigned long long to,
    ...
    dpi_hs_callback_context_t *ctx = (dpi_hs_callback_context_t *)hs_ctx;
