...
int dpi_cache_packet(dpi_packet_t *p, dpi_wing_t *w, bool lookup)
{
    ...
    clip = (clip_t *)malloc(sizeof(*clip) + p->raw.len);
    if (clip == NULL) {
        return -1;
    }
...
dpi_session_t *dpi_session_lookup(dpi_packet_t *p)
{
    ...
    if (likely(p->eth_type == ETH_P_IP)) {
        ...
        if (isproxymesh) {
            s = (dpi_session_t *)rcu_map_lookup(&th_session4_proxymesh_map, &key);
        } else {
            s = (dpi_session_t *)rcu_map_lookup(&th_session4_map, &key);
        }
    } else {
        ...
        if (isproxymesh) {
            s = (dpi_session_t *)rcu_map_lookup(&th_session6_proxymesh_map, &key);
        } else {
            s = (dpi_session_t *)rcu_map_lookup(&th_session6_map, &key);
        }
    }
...
void dpi_session_release(dpi_session_t *s)
{
    ...
    bool isproxymesh = cmp_mac_prefix(ep_mac, (char *)PROXYMESH_MAC_PREFIX);
...
static dpi_session_t *dpi_session_create(dpi_packet_t *p, bool to_server)
{
    ...
    dpi_session_t *s = (dpi_session_t *)calloc(1, sizeof(*s));
    if (unlikely(s == NULL)) {
        return NULL;
    }
...
void dpi_icmp_tracker(dpi_packet_t *p)
{
    ...
    if (dpi_threat_status(DPI_THRT_ICMP_FLOOD) && dpi_is_client_pkt(p)) {
        act = (dpi_meter_action_t)dpi_meter_packet_inc(DPI_METER_ICMP_FLOOD, p);
...
static const char *tcp_state_name[] = {
        tcp_state_name[TCP_ESTABLISHED] = "ESTABLISHED",
        tcp_state_name[TCP_SYN_SENT] =  "SYN_SENT",
        tcp_state_name[TCP_SYN_RECV] =  "SYN_RECV",
        tcp_state_name[TCP_FIN_WAIT1] =  "FIN_WAIT1",
        tcp_state_name[TCP_FIN_WAIT2] = "FIN_WAIT2",
        tcp_state_name[TCP_TIME_WAIT] = "TIME_WAIT",
        tcp_state_name[TCP_CLOSE] =    "CLOSE",
        tcp_state_name[TCP_CLOSE_WAIT] = "CLOSE_WAIT",
        tcp_state_name[TCP_LAST_ACK] =  "LAST_ACK",
        tcp_state_name[TCP_LISTEN]  =   "LISTEN",
        tcp_state_name[TCP_CLOSING]  =  "CLOSING",
};
...
static const char *tcp_event_name[] = {
        tcp_event_name[TCP_EVT_NORMAL] = "ACK",
        tcp_event_name[TCP_EVT_SYN] =   "SYN",
        tcp_event_name[TCP_EVT_SYNACK] = "SYNACK",
        tcp_event_name[TCP_EVT_TWH] =   "HANDSHAKE",
        tcp_event_name[TCP_EVT_SPLIT] = "SPLIT",
        tcp_event_name[TCP_EVT_FIN] =   "FIN",
        tcp_event_name[TCP_EVT_RST]  =  "RESET",
};
