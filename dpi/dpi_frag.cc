...
static int ip4frag_trac_match(struct cds_lfht_node *ht_node, const void *key)
{
    ...
    const ip4frag_trac_t *t2 = (ip4frag_trac_t *)key;
...
{
    const ip4frag_trac_t *t = (ip4frag_trac_t *)key;
...
{
    teardrop_args_t *td = (teardrop_args_t *)args;
...
static void ipfrag_hold(ip4frag_trac_t *trac, dpi_packet_t *p)
{
    ...
    // Save the fragment
    clip_t *clip = (clip_t *)malloc(sizeof(*clip) + p->cap_len);
    if (clip == NULL) {
        return;
    }
...
int dpi_ip_defrag(dpi_packet_t *p)
{
    ...
    trac = (ip4frag_trac_t *)rcu_map_lookup(&th_ip4frag_map, &key);
    if (trac == NULL) {
        trac = (ip4frag_trac_t *)malloc(sizeof(*trac));
        if (trac == NULL) {
            return -1;
        }
...
static int ip6frag_trac_match(struct cds_lfht_node *ht_node, const void *key)
{
    ...
    const ip6frag_trac_t *t2 = (ip6frag_trac_t *)key;
...
{
    const ip6frag_trac_t *t = (ip6frag_trac_t *)key;
...
int dpi_ipv6_defrag(dpi_packet_t *p)
{
    ...
    trac = (ip6frag_trac_t *)rcu_map_lookup(&th_ip6frag_map, &key);
    if (trac == NULL) {
        ...
        trac = (ip6frag_trac_t *)malloc(sizeof(*trac));
        if (trac == NULL) {
            return -1;
        }
...
{
    g_io_callback->send_packet((io_ctx_t *)args, clip->ptr, clip->len + clip->skip);
}

void dpi_frag_discard(void *frag_trac)
{
    frag_trac_t *trac = (frag_trac_t *)frag_trac;
    asm_destroy(&trac->frags, ipfrag_remove);
    free(trac);
}
